...
def make_recognize_ingredients_use_case(db):
    return RecognizeIngredientsUseCase(GeminiAdapterService(), RecognitionRepositoryImpl(db))

def make_recognize_foods_use_case(db):
    return RecognizeFoodsUseCase(GeminiAdapterService(), RecognitionRepositoryImpl(db))

def make_recognize_batch_use_case(db):
    return RecognizeBatchUseCase(GeminiAdapterService(), RecognitionRepositoryImpl(db))