...
from src.shared.exceptions.custom import MealPlanNotFoundException, InvalidRequestDataException
...
class UpdateMealPlanUseCase:
    ...
    def execute(self, user_uid: str, plan_date: date, meals: List[dict]) -> DailyMealPlan:
        ...
        if not existing_plan:
            raise MealPlanNotFoundException(f"No hay plan para el {plan_date} para modificar.")

        # Validar que no haya duplicados en categorías
        categories_seen = set()
        for meal in meals:
        ...
        self.meal_plan_repository.update_by_user_and_date(updated_plan)
        return updated_plan
