...
class GetEnvironmentalCalculationsByUserAndStatus:
    def __init__(self, savings_repository):
        self._savings_repo = savings_repository

    def execute(self, user_uid: str, is_cooked: bool) -> List[Dict]:
        filtered: List[EnvironmentalSavings] = self._savings_repo.find_by_user_and_by_is_cooked(user_uid, is_cooked)
        return [
            {
